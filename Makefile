...
%-resume.pdf:
	(for file in ./shared/*.md; do cat "$$file"; echo -e "\n"; done) | pandoc --metadata=lang=$* --metadata-file=resume.yaml --lua-filter=filter-ex.lua --lua-filter=filter-lang.lua --lua-filter columns.lua -f markdown+yaml_metadata_block --template templates/jb2-modern.latex -o $(OUTPUT_DIR)/$@
